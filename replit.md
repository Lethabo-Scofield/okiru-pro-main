...
- **Template architecture**: Two template systems exist — (1) MongoDB extraction templates (18 starter templates defining entities for document processing, used by Dashboard/DocumentProcessor), and (2) ArangoDB scorecard toolkits (Excel formula graphs for scoring, 6 ontology templates across 4 sectors: RCOGP, ICT, FSC, AGRI). The EntityBuilder fetches from `/api/entity-templates` (handled directly by web server, no longer proxied to API server) showing ontology toolkit templates with sector badges, pillar counts, and entity counts, plus user-saved templates. These ontology templates are read-only — users can load and customize entities but must "Publish as New" to save. Sector preset buttons use `/api/manifest` (proxied) to load entities from pillar packs. When ArangoDB is unavailable, the DocumentProcessor gracefully skips manifest-based sector templates and uses only the MongoDB extraction templates
