...
# Check arango container logs first
echo "=== ArangoDB logs ==="
docker logs okiru_arango 2>&1 | tail -30 || echo "No logs yet"

# Fix docker-compose: remove curl healthcheck for arango (curl not in image)
...
services:
  ...
  arangodb:
    ...
    ports:
      - "127.0.0.1:8529:8529"
    healthcheck:
      test: ["CMD-SHELL", "wget -qO- http://localhost:8529/_api/version || exit 1"]
      interval: 15s
      timeout: 10s
      retries: 10
      start_period: 30s
...
echo "=== Waiting 60s for everything to start ==="
sleep 60
...
docker logs okiru_arango 2>&1 | tail -10
docker logs okiru_api 2>&1 | tail -10
docker logs okiru_web 2>&1 | tail -10
