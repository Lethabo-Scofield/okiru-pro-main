...
| R22 | **HIGH** | ICT QSE / skills | identical to R8 — `ICT_QSE` absorption target still `1.0` (=1%) at `sectorConfig.ts:1260`; Excel score formula is the same `MIN(absorbed/completers × max, max)` 100% shape. ict-qse-golden.test.ts L83/L479 still assert the wrong 1% behaviour. | Set 1.0→100; correct the two ict-qse golden assertions exactly as R8 did | OPEN |
