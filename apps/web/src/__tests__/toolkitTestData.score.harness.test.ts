  ...
  it(`scores all ${files.length} workbooks; reports Level-1 fitness`, () => {
    ...
    for (const f of files) {
      ...
      try {
        ...
        // The level is the points, mirroring the live store. The deemed-level
        // floor that used to be applied here was removed with the rule itself:
        // the affidavit route is a verification outcome, not a measurement.
