...
import { ESG_THR_LTIFR } from "@/lib/esgScoringDefaults";
...
export function scoreSocial(workbook: EsgWorkbookData): SocialScoreResult {
  ...
  const thrLtifr = readEsgCell(workbook, "assumptions", "B55") ?? ESG_THR_LTIFR;

  const blackPct = readEsgCell(workbook, "ee", "B5") ?? 0;
  const thrBlack = readEsgCell(workbook, "assumptions", "B50") ?? 0.6;
  const d5 = pr(blackPct, thrBlack, 8, floor);

  const d6 = 0;
  const d7 = yesPartialNo(str(workbook, "B9", "ee"), 5);
  const pwd = readEsgCell(workbook, "ee", "B8") ?? 0;
  const d8 = pr(pwd, readEsgCell(workbook, "assumptions", "B52") ?? 0.02, 5, floor);
  ...
  const d14 = headcount > 0 ? pr(num(workbook, "B49") / headcount, readEsgCell(workbook, "assumptions", "B53") ?? 40, 5, floor) : 0;
  const levy = num(workbook, "B44");
  const d15 =
    levy > 0
      ? pr(num(workbook, "B47") / levy, readEsgCell(workbook, "assumptions", "B54") ?? 0.8, 5, floor)
      : 0;
  ...
  const initiatives =
    readEsgCell(workbook, "s-data", "_initiatives_count") ?? 6;
  const d23 = initiatives >= 6 ? 5 : pr(initiatives, 6, 5, floor);
