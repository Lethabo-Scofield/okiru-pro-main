/**
 * FSC Banks (FS701) — CalculatorConfig for Toolkit scoring.
 *
 * Key differences from FSC Generic (Others):
 * - Grand total: 130 pts (120 Others − 2 stockbroker bonus + 12 AFS)
 * - Empowerment Financing pillar: EF Targeted Investments + Transaction Financing
 *   Point values = 0 (Q44: blank in template when sub-sector = Others).
 * - SD target: 1.8% NPAT (vs 2% Others)
 * - ED target: 0.2% NPAT (vs 1% Others); no stockbroker bonus row on Banks EF&ESD sheet
 * - AFS: 12 pts — Transaction Point(5km,1pt) + Service Point(10km,1pt) +
 *   Sales Point(15km,2pts) + Electronic Access(2pts) + PoP(3pts) + Active Accounts(3pts)
 *
 * EF evidence: "EF & ESD Scorecard - Banks" PDF (rendered 2026-06-01):
 *   Applicable Sub-Sector Codes = Others → all EF point cells = 0.
 *   SD 1.80%, ED 0.20%, Graduation 100%, Jobs 100% (confirmed % targets).
 *   EF Targeted Investments and Transaction Financing points not available (Q44).
 *
 * @see docs/domain/sectors/fsc/generic/sls.md §6.8, §6.10, §12
 * @see docs/domain/sectors/EXPERT-QUESTIONS.md Q44
 */
import {
  FSC_BANKS,
  type SectorConfig,
...
export const FSC_BANKS_SECTOR_CODE = 'FSC' as const;
export const FSC_BANKS_SCORECARD_TYPE = 'Generic' as const;
export const FSC_BANKS_SUB_SECTOR = 'Banks' as const;

/** Convert FSC_BANKS SectorConfig → CalculatorConfig. */
export function sectorConfigToFscBanksCalculatorConfig(sc: SectorConfig): CalculatorConfig {
  const t = sc.targets;
  ...
  return {
    ...
    esd: {
      ...
      edJobsBonusMax: esd.edJobsBonus,              // 1 (no stockbroker for Banks)
      // Banks: no stockbroker bonus (not on Banks EF&ESD sheet)
    ...
    empowermentFinancing: {
      ...
      jobsBonusMaxPts: esd.edJobsBonus,
      stockbrokerBonusMaxPts: 0,
      stockbrokerTarget: 0,
    },

    /**
     * AFS Banks — 12 pts (verified from AFS Scorecard - Banks sheet, rendered 2026-06-01).
     * Structure: 6 indicators with geographic / electronic access criteria.
     */
    accessToFinancialServices: {
      subSector: 'Banks',
      maxPoints: pAfs?.maxPoints ?? 12,
      transactionPointTarget: 0.85,   // 85% of qualifying area with TP within 5km
      transactionPointMaxPts: 1,
      servicePointTarget: 0.70,       // 70% within 10km
      servicePointMaxPts: 1,
      salesPointTarget: 0.60,         // 60% within 15km
      salesPointMaxPts: 2,
      electronicAccessMaxPts: 2,      // national — yes/no
      pointOfPresenceMaxPts: 3,       // at least one PoP per measurement unit
      activeAccountsMaxPts: 3,        // active accounts in target market
    },
...
/** Complete FSC Banks (FS701) config. */
export const FSC_BANKS_CALCULATOR_CONFIG: CalculatorConfig =
  sectorConfigToFscBanksCalculatorConfig(FSC_BANKS);

/** Re-export per-level targets for test assertions. */
export { FSC_SKILLS_PER_LEVEL_TARGETS as FSC_BANKS_SKILLS_PER_LEVEL_TARGETS };

export function isFscBanksSector(sectorCode: string, fscSubSector?: string): boolean {
  return sectorCode.toUpperCase() === 'FSC' && normalizeFscSubSector(fscSubSector) === 'Banks';
}
