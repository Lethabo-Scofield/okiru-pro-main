 ...
 *   - percentage              ΓÇö actual % vs target %, prorated up to weight
 *   - percentage_of_npat      ΓÇö actual ZAR ├╖ npat ├ù 100, vs target %
 *   - percentage_of_leviable  ΓÇö actual ZAR ├╖ leviableAmount ├ù 100
 *   - percentage_of_tmps      ΓÇö actual ZAR ├╖ totalMeasuredProcurementSpend ├ù 100
 *   - bonus_threshold         ΓÇö full weight if actual ΓëÑ target, else 0
 *   - evidence                ΓÇö full weight if true, else 0
 *   - net_value               ΓÇö weight ├ù realisation factor (0ΓÇô1)
 *   - eap_percentage          ΓÇö actual % vs (africanEapPercent ├ù target)
 *
 ...
 *     in ZAR. Optional individually ΓÇö the engine downgrades only the
 *     indicators that depend on the missing field.
 *   - africanEapPercent: number ΓÇö Stats SA EAP %, optional (only used by
 *     `eap_percentage` indicators; defaults to a sane fallback when absent).
 *
 * Missing-data return contract:
 *   - If the indicator's `inputKey` is absent ΓåÆ status: "missing_data",
 *     achievedPoints: 0, actual: null, missingFields: [<inputKey>].
 *   - If a required financial is absent ΓåÆ status: "missing_data",
...
/** National EAP ΓÇö African People share (Stats SA EAP 2023, ~80.4%). Used as default for Skills "African People (per Stats SA EAP)". */
...
function recommend(indicator: ConstructionIndicator, status: IndicatorStatus, gap: number): string | undefined {
  ...
  if (status === 'missing_data') {
    return `Provide value for "${indicator.name}" ΓÇö required to score this indicator.`;
  }
